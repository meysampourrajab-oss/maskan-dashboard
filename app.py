...
conn = sqlite3.connect('housing_justice_v15.db', check_same_thread=False)
cur = conn.cursor()
...
# تابع تخصیص: اولویت ۱ مبلغ | اولویت ۲ دهک ۱ تا ۳
def calculate_allocation_priority(df_m, df_b):
    if df_m.empty or df_b.empty: return {}, {}, pd.DataFrame()
    
    # وزن‌دهی برای دهک‌های ۱ تا ۳ (فقط برای مبالغ مساوی)
    ...
    for i, m in m_sorted.iterrows():
        if b_ptr < len(b_sorted):
            curr_b = b_sorted.iloc[b_ptr]; b_id = int(curr_b['b_id'])
            mapping[m['name']] = b_id
            res_list.append({
            ...
            b_map_usage[b_id] += 1; fill += 1
            if fill >= int(curr_b['cap']): b_ptr += 1; fill = 0
        else:
...
if mode == "⚙️ پنل مدیریت و آپلود":
    st.title("⚙️ تنظیمات و بارگذاری")
    t1, t2, t3 = st.tabs(["🏗️ پروژه", "🏢 شرکت", "📤 آپلود فایل"])
    
    with t1:
        c1, c2 = st.columns(2)
        p_n, p_l = c1.text_input("نام پروژه"), c2.text_input("محل اجرا")
        if st.button("ثبت پروژه"):
            if p_n: cur.execute("INSERT INTO projects (name, location) VALUES (?,?)", (p_n, p_l)); conn.commit(); st.rerun()
        st.dataframe(all_projects, width='stretch', hide_index=True)

    with t2:
        if not all_projects.empty:
            sel_p = st.selectbox("انتخاب پروژه:", all_projects['name'].tolist())
            p_id = int(all_projects[all_projects['name'] == sel_p]['id'].iloc[0])
            c1, c2 = st.columns(2)
            c_n, c_u = c1.text_input("نام شرکت"), c2.number_input("تعداد واحد", min_value=1)
            if st.button("ثبت شرکت"):
                if c_n: cur.execute("INSERT INTO companies (p_id, name, units) VALUES (?,?,?)", (p_id, c_n, c_u)); conn.commit(); st.rerun()
    ...
    with t3:
        if not all_projects.empty:
            ...
            if not comps.empty:
                sel_c_up = st.selectbox("شرکت پیمانکار:", comps['name'].tolist())
                c_id_up = int(comps[comps['name'] == sel_c_up]['id'].iloc[0])
                m, y = st.selectbox("ماه:", ["فروردین","اردیبهشت","خرداد","تیر","مرداد","شهریور","مهر","آبان","آذر","دی","بهمن","اسفند"]), st.selectbox("سال:", [1404, 1405])
                f_date = f"{m} {y}"
                f_m, f_b = st.file_uploader("فایل واریزی", type=['csv','xlsx']), st.file_uploader("وضعیت بلوک‌ها", type=['csv','xlsx'])
                if st.button("🚀 ذخیره داده‌ها"):
                    if f_m and f_b:
                        ...
                        conn.commit(); st.success("اطلاعات با موفقیت ثبت شد.")

# --- بخش داشبورد مدیریتی ---
else:
    st.title("📊 داشبورد تحلیل و گزارشات سامانه")
    
    
    if all_projects.empty: st.info("پروژه‌ای موجود نیست.")
    else:
        c1, c2 = st.columns(2)
        p_sel = c1.selectbox("انتخاب پروژه:", all_projects['name'].tolist())
        p_id_v = int(all_projects[all_projects['name'] == p_sel]['id'].iloc[0])
        comps_v = pd.read_sql(f"SELECT * FROM companies WHERE p_id={p_id_v}", conn)
        
        if not comps_v.empty:
            c_sel = c2.selectbox("انتخاب شرکت پیمانکار:", comps_v['name'].tolist())
            c_id_v = int(comps_v[comps_v['name'] == c_sel]['id'].iloc[0])
            ...
            if months:
                ...
                # مقایسه با ماه قبل (فقط در صورت وجود داده)
                db_prev_map = {}
                df_b_prev = pd.DataFrame()
                if prev_m:
                    df_m_p = pd.read_sql(f"SELECT * FROM members WHERE c_id={c_id_v} AND month='{prev_m}'", conn)
                    df_b_p = pd.read_sql(f"SELECT * FROM blocks WHERE c_id={c_id_v} AND month='{prev_m}'", conn)
                    db_prev_map, _, _ = calculate_allocation_priority(df_m_p, df_b_p)
                    df_b_prev = df_b_p
                
                curr_map, b_use, df_res = calculate_allocation_priority(df_m, df_b)
                
                # ۱. آمار کلیدی (جلوگیری از RangeError با کنترل Delta)
                st.divider()
                st.subheader("📋 آمار تجمیعی")
                k1, k2, k3, k4 = st.columns(4)
                
                k1.metric("تعداد بلوک", len(df_b))
                k1.metric("کل واحدها", int(df_b['cap'].sum()))
                
                avg_p = df_b['prog'].mean()
                delta_val = f"{avg_p - df_b_prev['prog'].mean():.1f}%" if not df_b_prev.empty else None
                k2.metric("میانگین پیشرفت", f"{avg_p:.1f}%", delta=delta_val)
                k2.metric("تعداد اعضا", len(df_res))
                
                if not df_b.empty:
                    leader = df_b.loc[df_b['prog'].idxmax()]
                    k3.metric("بلوک پیشرو", f"شماره {int(leader['b_id'])}")
                    k3.metric("بیشترین پیشرفت", f"{leader['prog']}%")
                
                k4.metric("واریزی (م.ت)", f"{(df_m['payment'].sum()/10000000):,.0f}")
                k4.metric("دوره", view_m)

                # ۲. تب‌ها
                tabs = st.tabs(["📋 لیست تخصیص", "📊 پیشرفت فیزیکی", "📂 ظرفیت بلوک‌ها", "💰 تحلیل واریزی"])
                
                with tabs[0]:
                    def get_pb(row):
                        if prev_m and row['نام عضو'] in db_prev_map: return db_prev_map[row['نام عضو']]
                        return row['file_prev']
                    df_res['بلوک قبلی'] = df_res.apply(get_pb, axis=1)
                    st.dataframe(df_res[['ردیف', 'نام عضو', 'واریزی (ریال)', 'دهک', 'بلوک جدید', 'بلوک قبلی', 'پیشرفت']], width='stretch', hide_index=True)
                
                with tabs[1]:
                    # جلوگیری از RangeError: فقط وقتی ماکزیمم > 0 باشد رسم کن
                    if not df_b.empty and df_b['prog'].max() > 0:
                        fig = px.bar(df_b, x='b_id', y='prog', text='prog', color_discrete_sequence=['#2ecc71'])
                        fig.update_layout(xaxis_type='category')
                        st.plotly_chart(fig, width='stretch')
                    else: st.warning("اطلاعات پیشرفت جهت رسم نمودار کافی نیست.")

                with tabs[2]:
                    # بازگشت ستون درصد پیشرفت
                    cap_table = [{'بلوک': int(b['b_id']), 'ظرفیت': int(b['cap']), 'تخصیص': b_use.get(int(b['b_id']),0), 
                                 'باقی': int(b['cap'])-b_use.get(int(b['b_id']),0), 'درصد پیشرفت': f"{b['prog']}%"} for _, b in df_b.iterrows()]
                    st.dataframe(pd.DataFrame(cap_table), width='stretch', hide_index=True)
                ...
                with tabs[3]:
                    ...
                    with cl2:
                        if cts['تعداد'].sum() > 0:
                            st.plotly_chart(px.pie(cts, values='تعداد', names='بازه', hole=0.4), width='stretch')
                        else: st.info("داده‌ای برای واریزی یافت نشد.")
            else: st.warning("ابتدا فایل‌های این شرکت را آپلود کنید.")