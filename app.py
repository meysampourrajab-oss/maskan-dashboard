...
def calculate_allocation_logic(df_m, df_b):
    ...
    for i, m in m_sorted.iterrows():
        if b_ptr < len(b_sorted):
            ...
            mapping[m['name']] = b_id
            res_list.append({'ردیف': i+1, 'نام عضو': m['name'], 'واریزی': m['payment'], 'دهک': m['decile'], 'بلوک جدید': b_id, 'پیشرفت': f"{curr_b['prog']}%", 'file_prev': m['file_prev_block']})
            b_map_usage[b_id] += 1; fill += 1
            if fill >= int(curr_b['cap']): b_ptr += 1; fill = 0
        else:
            mapping[m['name']] = "عدم تخصیص"
            res_list.append({'ردیف': i+1, 'نام عضو': m['name'], 'واریزی': m['payment'], 'دهک': m['decile'], 'بلوک جدید': "عدم تخصیص", 'پیشرفت': "-", 'file_prev': m['file_prev_block']})
    ...
    if admin_password == "1234": # رمز عبور را اینجا تغییر دهید
        ...
        with t2:
            if not all_projects.empty:
                ...
                c1, c2 = st.columns(2)
                c_n, c_u = c1.text_input("نام شرکت"), c2.number_input("واحد", min_value=1)
        ...
        with t3:
            if not all_projects.empty:
                sel_p_up = st.selectbox("پروژه جهت آپلود:", all_projects['name'].tolist(), key="up_p")
                ...
                if not comps.empty:
                    ...
                    f_date, f_m, f_b = f"{m} {y}", st.file_uploader("واریزی"), st.file_uploader("بلوک‌ها")
                    if st.button("🚀 ذخیره"):
                        if f_m and f_b:
                            ...
                            for _, r in df_m.iterrows():
                                ...
                                cur.execute("INSERT INTO members (c_id, month, name, payment, decile, file_prev_block) VALUES (?,?,?,?,?,?)", (c_id_up, f_date, r.iloc[1], float(str(r.iloc[2]).replace(',','')), dec, pre))
                            for _, r in df_b.iterrows():
                                cur.execute("INSERT INTO blocks (c_id, month, b_id, prog, cap) VALUES (?,?,?,?,?)", (c_id_up, f_date, int(r.iloc[0]), float(str(r.iloc[1]).replace('%','')), int(r.iloc[2])))
                            conn.commit(); st.success("آپلود شد.")
        with t4:
            st.warning("در این بخش می‌توانید گزارشات ماهانه شرکت‌ها را حذف کنید.")
...
# --- بخش داشبورد مدیریتی ---
    ...
    if all_projects.empty: st.info("داده‌ای موجود نیست.")
    else:
        c1, c2 = st.columns(2)
        p_name = c1.selectbox("پروژه:", all_projects['name'].tolist())
        p_id_v = int(all_projects[all_projects['name'] == p_name]['id'].iloc[0])
        comps_v = pd.read_sql(f"SELECT * FROM companies WHERE p_id={p_id_v}", conn)
        if not comps_v.empty:
            c_name = c2.selectbox("شرکت:", comps_v['name'].tolist())
            c_id_v = int(comps_v[comps_v['name'] == c_name]['id'].iloc[0])
            months = pd.read_sql(f"SELECT DISTINCT month FROM blocks WHERE c_id={c_id_v}", conn)['month'].tolist()
            if months:
                view_m = st.select_slider("دوره:", options=months, value=months[-1])
                ...
                # رفع RangeError: اگر همه داده‌ها صفر باشند، نمودار نکش
                can_draw_prog = not df_b.empty and df_b['prog'].max() > 0
                ...
                if prev_m:
                    ...
                    prev_map, _, _ = calculate_allocation_logic(df_m_old, df_b_old)
                    df_b_p = df_b_old

                _, b_usage, df_res = calculate_allocation_logic(df_m, df_b)
                
                st.divider()
                st.subheader("📋 آمار کلیدی")
                ...
                if can_draw_prog:
                    best = df_b.loc[df_b['prog'].idxmax()]
                    k3.metric("بلوک پیشرو", f"شماره {int(best['b_id'])}"); k3.metric("پیشرفت آن", f"{best['prog']}%")
                k4.metric("واریزی (م.ت)", f"{(df_m['payment'].sum()/10000000):,.0f}"); k4.metric("دوره", view_m)

                tabs = st.tabs(["📋 لیست تخصیص", "📊 پیشرفت فیزیکی", "📂 ظرفیت بلوک", "💰 تحلیل مالی"])
                with tabs[0]:
                    def get_p(row):
                        if prev_m and row['نام عضو'] in prev_map: return prev_map[row['نام عضو']]
                        return row['file_prev']
                    df_res['بلوک قبلی'] = df_res.apply(get_p, axis=1)
                    st.dataframe(df_res[['ردیف', 'نام عضو', 'واریزی', 'دهک', 'بلوک جدید', 'بلوک قبلی', 'پیشرفت']], width=1200, hide_index=True)
                with tabs[1]:
                    if can_draw_prog:
                        fig = px.bar(df_b, x='b_id', y='prog', text='prog', color_discrete_sequence=['#2ecc71'])
                        fig.update_layout(xaxis_type='category'); st.plotly_chart(fig, width=1200)
                    else: st.warning("پیشرفت بلوک‌ها صفر است.")
                with tabs[2]:
                ...
                with tabs[3]:
                    ...
                    with cl2:
                        if cts['تعداد'].sum() > 0: st.plotly_chart(px.pie(cts, values='تعداد', names='بازه', hole=0.4), width=1200)
            else: st.warning("داده‌ای یافت نشد.")
